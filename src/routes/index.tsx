...
import {
  ShoppingBag, Star, ArrowRight, Truck, ShieldCheck, Sparkles,
  AlertCircle, Frown, Droplets, Power, Wind, Ruler, Heart, Usb,
  Lock, ChevronDown, CreditCard,
} from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="gradient-rose text-white text-center text-xs sm:text-sm py-2.5 px-4 font-medium tracking-wide">
        Livraison gratuite aujourd'hui <span aria-hidden>🚚</span> · Garantie 30 jours satisfait ou remboursé
      </div>
      ...
      <header className="sticky top-0 z-40 bg-background/80 backdrop-blur-md border-b border-border">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <button className="relative p-2 rounded-full hover:bg-secondary transition-colors" aria-label="Panier">
            ...
            <span className="absolute -top-0.5 -right-0.5 h-4 w-4 rounded-full bg-rose-gold text-[10px] text-white flex items-center justify-center font-semibold">1</span>
          </button>
        </div>
      </header>
          ...
          <Reveal>
            <div className="space-y-7">
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-6xl leading-[1.05] font-medium">
                Une peau parfaite commence par des <em className="text-rose-gold not-italic">pinceaux propres.</em>
              </h1>
              <p className="text-base sm:text-lg text-muted-foreground max-w-xl leading-relaxed">
                Nettoyez et séchez vos pinceaux de maquillage en moins de 30 secondes. Éliminez 99% des bactéries causant l'acné.
              </p>
              <div className="flex items-center gap-3">
                <Stars />
                <span className="text-sm text-muted-foreground">Recommandé par <strong className="text-foreground">+10 000</strong> passionnées de beauté</span>
              </div>
              <a
                href="#buy"
              ...
              <div className="flex flex-wrap gap-6 pt-2 text-xs text-muted-foreground">
                <span className="inline-flex items-center gap-1.5"><Truck className="h-4 w-4 text-rose-gold" /> Livraison offerte</span>
                <span className="inline-flex items-center gap-1.5"><ShieldCheck className="h-4 w-4 text-rose-gold" /> Garantie 30 jours</span>
                <span className="inline-flex items-center gap-1.5"><Lock className="h-4 w-4 text-rose-gold" /> Paiement sécurisé</span>
              </div>
            </div>
          </Reveal>
          <Reveal delay={150}>
          ...
          <Reveal>
            <div className="text-center max-w-2xl mx-auto mb-14">
              <h2 className="text-3xl sm:text-4xl lg:text-5xl font-medium leading-tight">
                Ne laissez plus vos pinceaux <em className="text-rose-gold not-italic">gâcher votre peau.</em>
              </h2>
            </div>
          </Reveal>
          <div className="grid md:grid-cols-3 gap-6">
            {[
              { icon: AlertCircle, title: "Le problème", text: "Les pinceaux sales accumulent sébum, poussière et bactéries au fil des jours." },
              { icon: Frown, title: "La conséquence", text: "Responsables de l'acné, des rougeurs et des pores obstrués sur votre peau." },
              { icon: Sparkles, title: "La solution", text: "Notre technologie rotative élimine 99% des impuretés en moins de 30 secondes." },
            ].map((c, i) => (
        ...
        <div className="max-w-7xl mx-auto px-6">
          <Reveal>
            <div className="text-center max-w-2xl mx-auto mb-16">
              <p className="text-xs uppercase tracking-[0.2em] text-rose-gold mb-3 font-semibold">Comment ça marche</p>
              <h2 className="text-3xl sm:text-4xl lg:text-5xl font-medium">Propre et sec en 3 étapes</h2>
            </div>
          </Reveal>
          <div className="grid md:grid-cols-3 gap-10 lg:gap-6">
            {[
              { n: "01", icon: Droplets, title: "Ajoutez", text: "Versez de l'eau tiède et une goutte de savon doux dans le bol." },
              { n: "02", icon: Power, title: "Activez", text: "Plongez le pinceau dans le collier silicone et lancez la rotation." },
              { n: "03", icon: Wind, title: "Séchez", text: "Retirez de l'eau et faites tourner pour un séchage instantané." },
            ].map((s, i) => (
              <Reveal key={s.n} delay={i * 120}>
                <div className="text-center">
                  <div className="relative inline-block mb-6">
                    <span className="font-display text-7xl lg:text-8xl text-accent/60 leading-none">{s.n}</span>
      ...
      <section className="bg-cream py-20 lg:py-28">
        <div className="max-w-7xl mx-auto px-6 space-y-20 lg:space-y-28">
          {[
            { icon: Ruler, title: "S'adapte à tous vos pinceaux", text: "Trois colliers en silicone multi-tailles épousent parfaitement chaque manche, du pinceau eyeliner au kabuki.", img: productMain, reverse: false },
            { icon: Heart, title: "Préserve la douceur des poils", text: "La rotation douce nettoie en profondeur sans abîmer les fibres délicates de vos pinceaux les plus précieux.", img: productLifestyle, reverse: true },
            { icon: Usb, title: "Rechargeable via USB-C", text: "Une batterie longue durée pour des semaines d'utilisation. Compact, sans fil, pensé pour votre coiffeuse.", img: productHero, reverse: false },
          ].map((f, i) => (
            <Reveal key={f.title}>
              <div className={`grid lg:grid-cols-2 gap-10 lg:gap-16 items-center ${f.reverse ? "lg:[&>div:first-child]:order-2" : ""}`}>
                <div>
      ...
      <section className="py-20 lg:py-28">
        <div className="max-w-7xl mx-auto px-6">
          <Reveal>
            <div className="text-center max-w-2xl mx-auto mb-14">
              <p className="text-xs uppercase tracking-[0.2em] text-rose-gold mb-3 font-semibold">Témoignages</p>
              <h2 className="text-3xl sm:text-4xl lg:text-5xl font-medium">Elles ont transformé leur routine</h2>
            </div>
          </Reveal>
          <div className="grid md:grid-cols-3 gap-6">
            {[
              { name: "Sophie M.", text: "Incroyable, mes pinceaux sèchent instantanément et je n'ai plus de boutons depuis que je l'utilise. Un must-have." },
              { name: "Camille L.", text: "Je détestais nettoyer mes pinceaux à la main. En 30 secondes c'est fait, ils sont comme neufs. Je recommande !" },
              { name: "Léa B.", text: "Le design est sublime sur ma coiffeuse et le résultat est bluffant. Ma peau me dit merci chaque matin." },
            ].map((t, i) => (
            ...
            <Reveal delay={120}>
              <div className="space-y-6">
                ...
                <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2 pt-2 text-xs text-muted-foreground">
                  <span className="inline-flex items-center gap-1.5"><Lock className="h-3.5 w-3.5" /> Paiement sécurisé</span>
                  <span className="inline-flex items-center gap-1.5"><CreditCard className="h-3.5 w-3.5" /> Visa · Mastercard · PayPal</span>
                  <span className="inline-flex items-center gap-1.5"><ShieldCheck className="h-3.5 w-3.5" /> Garantie 30 jours</span>
                </div>
              </div>
            </Reveal>
          </div>
        ...
        <div className="max-w-3xl mx-auto px-6">
          <Reveal>
            <div className="text-center mb-12">
              <p className="text-xs uppercase tracking-[0.2em] text-rose-gold mb-3 font-semibold">FAQ</p>
              <h2 className="text-3xl sm:text-4xl lg:text-5xl font-medium">Vos questions, nos réponses</h2>
            </div>
          </Reveal>
          <Accordion type="single" collapsible className="w-full space-y-3">
            {[
              { q: "Quels types de savons puis-je utiliser ?", a: "Un savon doux (savon de Marseille liquide, shampoing bébé ou nettoyant pinceaux dédié) avec de l'eau tiède suffit largement." },
              { q: "Combien de temps dure la batterie ?", a: "La batterie offre jusqu'à 90 nettoyages complets sur une seule charge USB-C, soit plusieurs semaines d'utilisation." },
              { q: "S'adapte-t-il à tous les pinceaux ?", a: "Oui — 3 colliers en silicone de tailles différentes sont fournis, compatibles avec pratiquement tous les pinceaux du marché." },
              { q: "Quand vais-je recevoir ma commande ?", a: "Livraison gratuite en 3 à 5 jours ouvrés partout en France métropolitaine, avec suivi du colis par e-mail." },
            ].map((item, i) => (
              <AccordionItem
                key={i}
                value={`item-${i}`}
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-8">
            ...
            <nav className="flex flex-wrap gap-x-8 gap-y-3 text-sm">
              {["Contact", "Politique de confidentialité", "CGV", "Retours"].map((l) => (
                <a key={l} href="#" className="hover:text-background transition-colors">{l}</a>
              ))}
            </nav>
          </div>
